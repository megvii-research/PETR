...
from mmdet3d.core.bbox import CameraInstance3DBoxes, get_box_type, LiDARInstance3DBoxes

@DATASETS.register_module()
class NuScenesDatasetMONO(Custom3DDataset):
    r"""NuScenes Dataset.
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            if (sample_id + 1) % CAM_NUM != 0:
                continue
            # print(det.keys())
            scores = torch.cat([det['scores_3d'] for det in boxes_per_frame], 0)
            labels = torch.cat([det['labels_3d'] for det in boxes_per_frame], 0)
            boxes_3d = torch.cat([det['boxes_3d'].tensor for det in boxes_per_frame], 0)

            scores, indexs = scores.topk(300)
            labels = labels[indexs]
            boxes_3d = boxes_3d[indexs]
            lidar_boxes3d = LiDARInstance3DBoxes(boxes_3d, box_dim=9)
            ...
            # boxes_3d, scores, labels = box3d_multiclass_nms(
            #     boxes3d,
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    if name in [
                        ...
                        attr = NuScenesDatasetMONO.DefaultAttribute[name]
                else:
