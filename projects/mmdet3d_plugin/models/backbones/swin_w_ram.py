...
class VidtTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if pretrained is not None:
            filename = "ckpts/"+pretrained.split("/")[-1]
    ...
    def forward(self, x, mask):
        ...
        for stage in range(self.num_layers):
            ...
            # if stage > 0:
                ...
                patch_out = norm_layer(patch_out).view(B, H, W, -1).permute(0, 3, 1, 2)
                # patch_out = patch_out.view(B, H, W, -1).permute(0, 3, 1, 2)
    ...
    def train(self, mode=True):
        ...
        super(VidtTransformer, self).train(mode)
        self._freeze_stages()
...
def swin_nano(pretrained=None, **kwargs):
    model = VidtTransformer(pretrain_img_size=[224, 224], embed_dim=48, depths=[2, 2, 6, 2],
...
def swin_tiny(pretrained=None, **kwargs):
    model = VidtTransformer(pretrain_img_size=[224, 224], embed_dim=96, depths=[2, 2, 6, 2],
...
def swin_small(pretrained=None, **kwargs):
    model = VidtTransformer(pretrain_img_size=[224, 224], embed_dim=96, depths=[2, 2, 18, 2],
...
def swin_base_win7(pretrained=None, **kwargs):
    model = VidtTransformer(pretrain_img_size=[224, 224], embed_dim=128, depths=[2, 2, 18, 2],
...
def swin_large_win7(pretrained=None, **kwargs):
    model = VidtTransformer(pretrain_img_size=[224, 224], embed_dim=192, depths=[2, 2, 18, 2],
