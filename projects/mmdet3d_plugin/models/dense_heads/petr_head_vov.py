...
class PETRHeadVOV(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRHeadVOV):
        ...
        self.num_pred = 6
        super(PETRHeadVOV, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def position_embeding(self, img_feats, img_metas, masks=None):
        eps = 1e-5
        self.position_range = [-65, -65, -8.0, 65, 65, 8.0]
        pad_h, pad_w, _ = img_metas[0]['pad_shape'][0]
        B, N, C, H, W = img_feats[self.position_level].shape
        coords_h = torch.arange(H, device=img_feats[0].device).float() * pad_h / H
        coords_w = torch.arange(W, device=img_feats[0].device).float() * pad_w / W
        coords_d = torch.arange(start=1.0, end=self.position_range[3], step=1.0, device=img_feats[0].device).float()
        D = coords_d.shape[0]
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRHeadVOV:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        if self.with_position:
            ...
            if self.with_multiview:
                ...
                sine_embed = torch.cat(pos_embeds, 1)
                pos_embed = pos_embed + sine_embed
        else:
            if self.with_multiview:
